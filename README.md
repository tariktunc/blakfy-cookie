...
| `data-blakfy-error-endpoint` | `null`                                              | URL                | Opsiyonel (#30 item 6). `bootstrap()` init sırasında throw ederse (banner hiç görünmeyecek) bu endpoint'e `{type:"bootstrap_error",message,timestamp}` POST edilir — aynı sendBeacon-first transport (#28). Verilmezse sadece `console.error` ile loud uyarı basılır, ağ isteği hiç yapılmaz. |
...
## Hata Modları (#30 item 6)

| Senaryo                                           | Sonuç                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cookie-defaults.min.js` erişilemez (404/timeout) | `<head>`'de render-blocking olsa da bu **tarayıcının kendi davranışıdır**: 404 olan bir `<script src>` sayfa render'ını durdurmaz, sadece o script çalışmaz. Consent Mode default sinyalleri kurulmaz — #43'ün diagnose() / "defaults did not run" uyarısı bunu tespit eder.                                                                                                              |
| Ana bundle (`cookie.min.js`) erişilemez           | Sayfa normal render olur, sadece consent UI hiç görünmez. Gated tag'ler **fail-closed** kalır (#30 item 5) — widget hiç çalışmadığı için `unblockScripts()`/`unblockIframes()` hiç çağrılmaz.                                                                                                                                                                                             |
| `bootstrap()` init sırasında throw ederse         | `runBootstrap()`'ın hatası artık sessiz bir "unhandled promise rejection" değil: `console.error` ile loud bir mesaj basılır (ne olduğunu ve tag'lerin bloklu kaldığını söyler) ve varsa `data-blakfy-error-endpoint`'e `{type:"bootstrap_error",message,timestamp}` POST edilir (opsiyonel, #28'in sendBeacon-first transport'unu tekrar kullanır). Gated tag'ler yine fail-closed kalır. |

Bkz. `tests/integration/bootstrap-error.test.js`.
